        ...
        candidateDb = mongoose.createConnection(url, options, function(error, result) {
            if (error) {
                ...
                defaultDb.on('disconnected', function() {
                    ...
                    var reconnectDelayMilliseconds = 5000;

                    var delay = Math.max(0, reconnectDelayMilliseconds - (now - lastReconnectAttempt));

                    logger.info(context, 'Reconnecting to MongoDB in ' + delay + "mills");

                    reconnectInterval = setInterval(function() {
                        connectionAttempt(url, options, callback);
                    }, delay);
                });
